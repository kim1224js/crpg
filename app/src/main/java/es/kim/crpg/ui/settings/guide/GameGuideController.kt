...
class GameGuideController(
    ...
    private fun expandedWeaponSource(grade: String): String = when (grade) {
        ...
        "LEGENDARY" -> "15층·20층·25층 보스·26~30층 상자"
        "MYTHIC" -> "20층·25층 보스·26~30층 몬스터·상자"
        else -> "던전 몬스터·상자"
    }
    ...
    private fun createRulesView(): View {
        val body = """
            ...
            [최종 층]
            26~30층은 신화 장비에 잠식된 가문이 지키는 선조의 왕릉입니다. 30층의 잊혀진 영주를 쓰러뜨리면 창고가 100칸으로 확장되고 엔딩 후 마을로 돌아갑니다.
        """.trimIndent()
