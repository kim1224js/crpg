...
class ItemGridView(
    ...
    private val capacity: Int = rows * columns,
    ...
    init {
        ...
        repeat(capacity.coerceAtMost(rows * columns)) { slotIndex ->
            addView(createSlot(slotIndex, bySlot[slotIndex]), slotParams(slotIndex))
        }
        ...
        HexagonSlotView(context, GameUiTheme.LEATHER_DARK, item?.let { gradeColor(it.itemCode) } ?: GameUiTheme.GOLD_DARK).apply {
            ...
            if (item != null) {
                addView(ImageView(context).apply {
                    ...
                    setImageBitmap(context.assets.open(assetPath(item)).use(BitmapFactory::decodeStream))
        ...
        when (event.action) {
            ...
            DragEvent.ACTION_DRAG_ENDED -> {
                ...
                repeat(childCount) { index ->
                    getChildAt(index).apply { alpha = 1f; scaleX = 1f; scaleY = 1f }
                }
            }
        }
        return true
    ...
    private fun resizeSlots(gridWidth: Int) {
        ...
        val slotWidth = ((gridWidth - dp(8) * columns) / columns).coerceIn(dp(36), dp(64))
